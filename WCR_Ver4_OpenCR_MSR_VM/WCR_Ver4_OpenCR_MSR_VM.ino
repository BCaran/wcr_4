...
void loop()
{
    ...
    if (startVMmsr == true)
    {
        ...
        if (logFile)
        {
            ...
            sendDataStreamToSD(0);
            for (int i = 0; i < num_of_arr_elements; i++)
            {
                if (startVMmsr == false)
                    break;

                tempVMarr[FL_POSITION] = aa08[i];
                tempVMarr[FR_POSITION] = aa11[i];
                tempVMarr[BL_POSITION] = aa09[i];
                tempVMarr[BR_POSITION] = aa10[i];

                tempVMarr[FL_WHEEL] = aa12[i];
                tempVMarr[FR_WHEEL] = aa15[i];
                tempVMarr[BL_WHEEL] = aa13[i];
                tempVMarr[BR_WHEEL] = aa14[i];

                motor_driver.sendVMcmd(tempVMarr);
                delay_ms(delay_time_msr);
                motor_driver.readEncodersPosition(encoderTicks);
                sendDataStreamToSD(i + 1);
            }
            logFile.println("*****************End of measurement*****************");
            logFile.close();

            tempVMarr[FL_WHEEL] = 0.0;
            tempVMarr[FL_POSITION] = 0.0;
            tempVMarr[FR_WHEEL] = 0.0;
            tempVMarr[FR_POSITION] = 0.0;
            tempVMarr[BL_WHEEL] = 0.0;
            tempVMarr[BL_POSITION] = 0.0;
            tempVMarr[BR_WHEEL] = 0.0;
            tempVMarr[BR_POSITION] = 0.0;
            motor_driver.sendVMcmd(tempVMarr);
            startVMmsr = false;
        }
    }
