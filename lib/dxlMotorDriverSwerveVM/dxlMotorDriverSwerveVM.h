...
#define RPM_FACTOR 0.229f
#define DEG_PER_PULS 0.087890625f // 4096/360 DEG per PULS
#define PULS_PER_DEG 11.3788f     // 360/4096 PULS per DEG
#define PULS_PER_RAD 651.8986f    // 2pi/4096  PULS per RAD
...
class dxlMotorDriverSwerveVM
{
public:
    dxlMotorDriverSwerveVM();
    ~dxlMotorDriverSwerveVM();
    bool init(void);
    ...
    bool sendVMcmd(float cmdArray[]);

    bool writeWheelSpeed(int64_t write_Wheel_Speed[]);
    bool writePositionSpeed(int64_t write_Position_Speed[]);
