...
dxlMotorDriverSwerveVM::dxlMotorDriverSwerveVM()
{
    baudrate_ = BAUDRATE;
...
bool dxlMotorDriverSwerveVM::init(void)
{
    // Serial.begin(57600);
...
bool dxlMotorDriverSwerveVM::setOperatingMode()
{
    uint8_t dxl_error = 0;
...
bool dxlMotorDriverSwerveVM::setPgainPositionMotors(uint16_t Pgain)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
...
bool dxlMotorDriverSwerveVM::setTorqueRotationMotors(bool onOff_wheels)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
...
bool dxlMotorDriverSwerveVM::setTorquePositionMotors(bool onOff_postion)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
...
bool dxlMotorDriverSwerveVM::readMotorsSpeed(int32_t motorsSpeedRead[])
{
...
bool dxlMotorDriverSwerveVM::readEncodersPosition(int32_t encodersPositionRead[])
{
    int dxl_comm_result = COMM_TX_FAIL; // Communication result
    bool dxl_addparam_result = false;   // addParam result
    bool dxl_getdata_result = false;    // GetParam result
...
bool dxlMotorDriverSwerveVM::writeWheelSpeed(int64_t write_Wheel_Speed[])
{
...
bool dxlMotorDriverSwerveVM::writeRotationPosition(int64_t write_Position_Motors[])
{
    bool dxl_addparam_result;
    int8_t dxl_comm_result;
    uint8_t FL_data_byte[4] = {
...
bool dxlMotorDriverSwerveVM::sendVMcmd(float cmdArray[])
{
    bool dxl_comm_result = false;
    float returnValues[8];
    ...
    returnValues[FL_POSITION] = cmdArray[FL_POSITION] * PULS_PER_RAD + (float)FL_zero_offset;
    returnValues[FR_POSITION] = cmdArray[FR_POSITION] * PULS_PER_RAD + (float)FR_zero_offset;
    returnValues[BL_POSITION] = cmdArray[BL_POSITION] * PULS_PER_RAD + (float)BL_zero_offset;
    returnValues[BR_POSITION] = cmdArray[BR_POSITION] * PULS_PER_RAD + (float)BR_zero_offset;
    ...
    returnValues[FL_WHEEL] = constrain(cmdArray[FL_WHEEL] * VELOCITY_CONSTANT_VALUE_WHEEL / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
    returnValues[FR_WHEEL] = constrain(cmdArray[FR_WHEEL] * VELOCITY_CONSTANT_VALUE_WHEEL / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
    returnValues[BL_WHEEL] = constrain(cmdArray[BL_WHEEL] * VELOCITY_CONSTANT_VALUE_WHEEL / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
    returnValues[BR_WHEEL] = constrain(cmdArray[BR_WHEEL] * VELOCITY_CONSTANT_VALUE_WHEEL / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
...
bool dxlMotorDriverSwerveVM::moveRobot(double x_vel, double y_vel, double z_rot)
{
    bool dxl_comm_result = false;
...
bool dxlMotorDriverSwerveVM::readRobotInfo(double returnRobotSpeeds[], int32_t returnWheelEncoderTicks[], double returnPositionMotorsRad[], double returnMotorSpeeds[])
{
...
void dxlMotorDriverSwerveVM::setZeroPosition()
{
    int64_t zeroPositon[4] = {FL_ZERO_POS, FR_ZERO_POS, BL_ZERO_POS, BR_ZERO_POS};
    writeRotationPosition(zeroPositon);
}

double dxlMotorDriverSwerveVM::mapf(double val, double in_min, double in_max, double out_min, double out_max)
{
