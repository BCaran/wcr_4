...
dxlMotorDriverSwerve::dxlMotorDriverSwerve()
{
    ...
    HCP_height_id_ = DXL_ID_HCP;
    GPR_height_id_ = DXL_ID_GPR_HEIGHT;
    GPR_rotation_id_ = DLX_ID_GPR_ROTATION;
...
bool dxlMotorDriverSwerve::init(void)
{
    ...
    //MEQ:
    //groupSyncReadMeasuringEquipment_ = new dynamixel::GroupSyncRead(portHandler_, packetHandler_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
   groupSyncWriteMeasuringEquipment_ = new dynamixel::GroupSyncWrite(portHandler_, packetHandler_, ADDR_X_GOAL_POSITION, LEN_X_GOAL_POSITION);
    ...
    //MEQ:
    setTorqueMeasuringEqMotors(true);
...
bool dxlMotorDriverSwerve::setOperatingMode()
{
    ...
    //MEQ:
    //HCP Measuring Equipment:
    /*
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, HCP_height_id_, ADDR_X_OPERATING_MODE, POSITION_CONTROL_MODE, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    ...
    */
    //GPR_Translation Measuring Equipment:
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, GPR_height_id_, ADDR_X_OPERATING_MODE, POSITION_CONTROL_MODE, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    ...
    //GPR_Rotation Measuring Equipment:
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, GPR_rotation_id_, ADDR_X_OPERATING_MODE, POSITION_CONTROL_MODE, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
...
bool dxlMotorDriverSwerve::setPgainPositionMotors(uint16_t Pgain)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
    ...
    //TODO SET P GAIN FOR HPC, GPR_TRANSLATION, GPR_ROTATION
    return true;
}
bool dxlMotorDriverSwerve::setTorqueRotationMotors(bool onOff_wheels)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
...
bool dxlMotorDriverSwerve::setTorquePositionMotors(bool onOff_postion)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;
...
//MEQ:
bool dxlMotorDriverSwerve::setTorqueMeasuringEqMotors (bool onOff_MeqEq)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;

    //HCP Measuring Equipment:
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, HCP_height_id_, ADDR_X_TORQUE_ENABLE, onOff_MeqEq, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    ...
    //GPR_Translation Measuring Equipment:
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, GPR_height_id_, ADDR_X_TORQUE_ENABLE, onOff_MeqEq, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        //  Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    ...
    //GPR_Rotation Measuring Equipment:
    dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, GPR_rotation_id_, ADDR_X_TORQUE_ENABLE, onOff_MeqEq, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
        //  Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
...
bool dxlMotorDriverSwerve::readMotorsSpeed(int32_t motorsSpeedRead[])
{
...
bool dxlMotorDriverSwerve::readEncodersPosition(int32_t encodersPositionRead[])
{
    int dxl_comm_result = COMM_TX_FAIL; // Communication result
    bool dxl_addparam_result = false;   // addParam result
    bool dxl_getdata_result = false;    // GetParam result
...
// MEQ:
bool dxlMotorDriverSwerve::readEncodersMeasuringEquipment(int32_t meqEqReadPosition[])
{
    int dxl_comm_result = COMM_TX_FAIL; // Communication result
    bool dxl_addparam_result = false;   // addParam result
    bool dxl_getdata_result = false;    // GetParam result
    ...
    // Set parameter
    /*
    dxl_addparam_result = groupSyncReadMeasuringEquipment_->addParam(HCP_height_id_);
    if (dxl_addparam_result != true)
        return false;
    
    dxl_addparam_result = groupSyncReadMeasuringEquipment_->addParam(GPR_height_id_);
    if (dxl_addparam_result != true)
        return false;
    dxl_addparam_result = groupSyncReadMeasuringEquipment_->addParam(GPR_rotation_id_);
    if (dxl_addparam_result != true)
        return false;

    // Syncread current Meq position
    dxl_comm_result = groupSyncReadMeasuringEquipment_->txRxPacket();
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    ...
    // Check if groupSyncRead data of Dynamixels are available
    /*
    dxl_getdata_result = groupSyncReadMeasuringEquipment_->isAvailable(HCP_height_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
    if (dxl_getdata_result != true)
        return false;
 
    dxl_getdata_result = groupSyncReadMeasuringEquipment_->isAvailable(GPR_height_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
    if (dxl_getdata_result != true)
        return false;
    dxl_getdata_result = groupSyncReadMeasuringEquipment_->isAvailable(GPR_rotation_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
    if (dxl_getdata_result != true)
        return false;

    // Get data
    //meqEqReadPosition[HCP_HEIGHT] = groupSyncReadMeasuringEquipment_->getData(HCP_height_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
    meqEqReadPosition[HCP_HEIGHT] = 0;
    meqEqReadPosition[GPR_HEIGHT] = groupSyncReadMeasuringEquipment_->getData(GPR_height_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
    meqEqReadPosition[GPR_ROTATION] = groupSyncReadMeasuringEquipment_->getData(GPR_rotation_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);

    groupSyncReadMeasuringEquipment_->clearParam();
       */
    meqEqReadPosition[HCP_HEIGHT] = 0;
    meqEqReadPosition[GPR_HEIGHT] = 0;
    meqEqReadPosition[GPR_ROTATION] = 0;  
    return true;
}

bool dxlMotorDriverSwerve::writeWheelSpeed(int64_t write_Wheel_Speed[])
{
...
bool dxlMotorDriverSwerve::writeRotationPosition(int64_t write_Position_Motors[])
{
    bool dxl_addparam_result;
    int8_t dxl_comm_result;
    uint8_t FL_data_byte[4] = {
...
//MEQ:
bool dxlMotorDriverSwerve::writeMeasuringEquipmentPosition(int64_t write_Meq_Position[])
{
    bool dxl_addparam_result;
    int8_t dxl_comm_result;
    uint8_t HCP_height_data_byte[4] = {
        0,
    };
    uint8_t GPR_height_data_byte[4] = {
        0,
    };
    uint8_t GPR_rotation_data_byte[4] = {
        0,
    };
    //HCP Measuring Equipment:
    HCP_height_data_byte[0] = DXL_LOBYTE(DXL_LOWORD(write_Meq_Position[0]));
    HCP_height_data_byte[1] = DXL_HIBYTE(DXL_LOWORD(write_Meq_Position[0]));
    HCP_height_data_byte[2] = DXL_LOBYTE(DXL_HIWORD(write_Meq_Position[0]));
    HCP_height_data_byte[3] = DXL_HIBYTE(DXL_HIWORD(write_Meq_Position[0]));
    /*
    dxl_addparam_result = groupSyncWriteMeasuringEquipment_->addParam(HCP_height_id_, (uint8_t *)&HCP_height_data_byte);
    if (dxl_addparam_result != true)
        return false;
    */
    //GPR_Translation Measuring Equipmentl:
    GPR_height_data_byte[0] = DXL_LOBYTE(DXL_LOWORD(write_Meq_Position[1]));
    GPR_height_data_byte[1] = DXL_HIBYTE(DXL_LOWORD(write_Meq_Position[1]));
    GPR_height_data_byte[2] = DXL_LOBYTE(DXL_HIWORD(write_Meq_Position[1]));
    GPR_height_data_byte[3] = DXL_HIBYTE(DXL_HIWORD(write_Meq_Position[1]));
    dxl_addparam_result = groupSyncWriteMeasuringEquipment_->addParam(GPR_height_id_, (uint8_t *)&GPR_height_data_byte);
    if (dxl_addparam_result != true)
        return false;
    //GPR_Rotation Measuring Equipment:
    GPR_rotation_data_byte[0] = DXL_LOBYTE(DXL_LOWORD(write_Meq_Position[2]));
    GPR_rotation_data_byte[1] = DXL_HIBYTE(DXL_LOWORD(write_Meq_Position[2]));
    GPR_rotation_data_byte[2] = DXL_LOBYTE(DXL_HIWORD(write_Meq_Position[2]));
    GPR_rotation_data_byte[3] = DXL_HIBYTE(DXL_HIWORD(write_Meq_Position[2]));
    dxl_addparam_result = groupSyncWriteMeasuringEquipment_->addParam(GPR_rotation_id_, (uint8_t *)&GPR_rotation_data_byte);
    if (dxl_addparam_result != true)
        return false;
    dxl_comm_result = groupSyncWriteMeasuringEquipment_->txPacket();
    if (dxl_comm_result != COMM_SUCCESS)
    {
        // Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
        return false;
    }
    groupSyncWriteMeasuringEquipment_->clearParam();
    return true;
}

bool dxlMotorDriverSwerve::setMeasuringEquipmentPosition(double meqGoalPosition[])
{
    bool dxl_comm_result = false;
    int64_t sendValues[3] = {0, 0, 0};

    meqGoalPosition[HCP_HEIGHT] = constrain(meqGoalPosition[HCP_HEIGHT], MIN_HEIGHT_HCP_MM, MAX_HEIGHT_HCP_MM);
    meqGoalPosition[GPR_HEIGHT] = constrain(meqGoalPosition[GPR_HEIGHT], MIN_HEIGHT_GPR_MM, MAX_HEIGHT_GPR_MM);
    meqGoalPosition[GPR_ROTATION] = constrain(meqGoalPosition[GPR_ROTATION], GPR_90_DEG_TO_RIGHT_DEG, GPR_90_DEG_TO_LEFT_DEG);
    
    sendValues[HCP_HEIGHT] = int64_t(mapf(meqGoalPosition[HCP_HEIGHT], MIN_HEIGHT_HCP_MM, MAX_HEIGHT_HCP_MM, MIN_HEIGHT_HCP, MAX_HEIGHT_HCP));
    sendValues[GPR_HEIGHT] = int64_t(mapf(meqGoalPosition[GPR_HEIGHT], MIN_HEIGHT_GPR_MM, MAX_HEIGHT_GPR_MM, MIN_HEIGHT_GPR, MAX_HEIGHT_GPR));
    sendValues[GPR_ROTATION] = int64_t(mapf(meqGoalPosition[GPR_ROTATION], GPR_90_DEG_TO_RIGHT_DEG, GPR_90_DEG_TO_LEFT_DEG, GPR_90_DEG_TO_RIGHT, GPR_90_DEG_TO_LEFT));   
    
    writeMeasuringEquipmentPosition(sendValues);
    dxl_comm_result = writeMeasuringEquipmentPosition(sendValues);
    if (dxl_comm_result == false)
        return 0;
}

bool dxlMotorDriverSwerve::returnCurrentMeasuringEquipementPosition(double meqCurrentPosition[])
{
    int32_t readEncoderTicks[3] = {0, 0, 0};
    readEncodersMeasuringEquipment(readEncoderTicks);

    meqCurrentPosition[HCP_HEIGHT] = mapf(double(readEncoderTicks[HCP_HEIGHT]), MIN_HEIGHT_HCP, MAX_HEIGHT_HCP, MIN_HEIGHT_HCP_MM, MAX_HEIGHT_HCP_MM);
    meqCurrentPosition[GPR_HEIGHT] = mapf(double(readEncoderTicks[GPR_HEIGHT]), MIN_HEIGHT_GPR, MAX_HEIGHT_GPR, MIN_HEIGHT_GPR_MM, MAX_HEIGHT_GPR_MM);
    meqCurrentPosition[GPR_ROTATION] = mapf(double(readEncoderTicks[GPR_ROTATION]), GPR_90_DEG_TO_RIGHT, GPR_90_DEG_TO_LEFT, GPR_90_DEG_TO_RIGHT_DEG, GPR_90_DEG_TO_LEFT_DEG);   
}

bool dxlMotorDriverSwerve::moveRobot(double x_vel, double y_vel, double z_rot)
{
    bool dxl_comm_result = false;
...
bool dxlMotorDriverSwerve::readRobotInfo(double returnRobotSpeeds[], int32_t returnWheelEncoderTicks[], double returnPositionMotorsRad[], double returnMotorSpeeds[])
{
...
void dxlMotorDriverSwerve::setZeroPosition()
{
    int64_t zeroPositon_Wheels[4] = {FL_ZERO_POS, FR_ZERO_POS, BL_ZERO_POS, BR_ZERO_POS};
    writeRotationPosition(zeroPositon_Wheels);
    
    //MEQ:
    int64_t zeroPositon_Meq[3] = {MAX_HEIGHT_HCP, MAX_HEIGHT_GPR, MIDDLE_GPR};
    writeMeasuringEquipmentPosition(zeroPositon_Meq);
}

double dxlMotorDriverSwerve::mapf(double val, double in_min, double in_max, double out_min, double out_max)
{
